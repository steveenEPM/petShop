...
export default function Productos() {
    ...
    */
    ...
    useMemo(() => {
        ...
        console.log(str)
        if (str === "/productos") {
            setElements(listPros)
            return
        }

        else if (str.match('/productos/search')) {
            const filter = listPros.filter(e =>{
                let nombre = e.nombre.toLocaleLowerCase()
                let descr = e.descripcion.toLocaleLowerCase()
                if((nombre.match(params.id.toLocaleLowerCase())) || (descr.match(params.id.toLocaleLowerCase())) ) return e
            })

            setElements(filter)
            return
        }else{
            const filter = listPros.filter(e => e.animal.toLocaleLowerCase() === params.id)
            setElements(filter)
